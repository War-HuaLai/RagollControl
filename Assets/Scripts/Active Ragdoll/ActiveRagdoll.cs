...
namespace ActiveRagdoll {
    ...
    [RequireComponent(typeof(InputModule))]//挂载的物体必须有InputModule组件
    public class ActiveRagdoll : MonoBehaviour {
        [Header("--- GENERAL ---")]
        [SerializeField] private int _solverIterations = 12;//迭代次数
        [SerializeField] private int _velSolverIterations = 4;
        [SerializeField] private float _maxAngularVelocity = 50;//最大角速度
        ...
        [Header("--- ANIMATORS ---")]
        [SerializeField] private Animator _animatedAnimator;//动画动画
        [SerializeField] private Animator _physicalAnimator;//物理动画
        ...
        public bool SyncTorsoPositions { get; set; } = true;//是否同步位置
        public bool SyncTorsoRotations { get; set; } = true;//是否同步旋转
        ...
        private List<ConfigurableJoint> TryGetJoints(params HumanBodyBones[] bones) {
            ...
            foreach (HumanBodyBones bone in bones) {
                Transform boneTransform = _physicalAnimator.GetBoneTransform(bone);//获取骨骼
        ...
        public Transform GetPhysicalBone(HumanBodyBones bone)//获得物理骨骼
        {
        ...
        public void SetStrengthScaleForAllBodyParts (float scale) //设置所有部位的强度
        {
            foreach (BodyPart bodyPart in _bodyParts)
