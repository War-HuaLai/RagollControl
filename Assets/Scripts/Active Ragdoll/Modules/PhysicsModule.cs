...
namespace ActiveRagdoll {
    ...
    public class PhysicsModule : Module {
        ...
        public float customTorsoAngularDrag = 0.05f;//自定义躯干角阻力

        [Header("--- UPRIGHT TORQUE ---")]
        public float uprightTorque = 10000;//竖直扭矩
        [Tooltip("Defines how much torque percent is applied given the inclination angle percent [0, 1]")]
        public AnimationCurve uprightTorqueFunction;//竖直扭矩函数
        public float rotationTorque = 500;//旋转扭矩

        [Header("--- MANUAL TORQUE ---")]
        public float manualTorque = 500;//手动扭矩
        public float maxManualRotSpeed = 5;//最大手动旋转速度

        private Vector2 _torqueInput;//扭矩输入

        [Header("--- STABILIZER JOINT ---")]
        [SerializeField] private JointDriveConfig _stabilizerJointDrive;//稳定器关节驱动
        ...
        private void Start() {
            UpdateTargetRotation();//更新目标旋转
            InitializeStabilizerJoint();//初始化稳定器关节
            StartBalance();//开始平衡
        }
        ...
        private void InitializeStabilizerJoint() {
            _stabilizerGameobject = new GameObject("Stabilizer", typeof(Rigidbody), typeof(ConfigurableJoint));//创建稳定器游戏对象
